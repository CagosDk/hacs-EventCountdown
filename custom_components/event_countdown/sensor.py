...
async def async_setup_entry(
    ...
    # Merge options on top of the original event data
    event = {**entry.data.get("event", {}), **entry.options}
    if not event.get("name"):
        return

    sensor = EventCountdownSensor(entry, event)
...
def _compute(event: dict) -> dict | None:
    ...
    if event_type == "fødselsdag":
        ...
        full_name = (
            f"{base} {age} års fødselsdag {dagstekst}" if age is not None
            else f"{base} fødselsdag {dagstekst}"
        )
    elif event_type == "bryllup":
        full_name = (
            f"{age} års {name.lower()} {dagstekst}" if age is not None
    ...
    event_date = (
        f"{year}-{month:02d}-{day:02d}" if isinstance(year, int)
...
class EventCountdownSensor(SensorEntity):
    ...
    def __init__(self, entry: ConfigEntry, event: dict) -> None:
        self._entry = entry
        self._event = event
        slug = re.sub(r"[^a-z0-9]+", "_", event["name"].lower()).strip("_")
